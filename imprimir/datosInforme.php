...
// PRECIO DE COMPRA
$sqlPrecioCompra = "SELECT precioCompra,fecha FROM registroingresos WHERE fecha BETWEEN '$desde' AND '$hasta' ORDER BY fecha ASC";

$queryPrecioCompra = mysqli_query($conexion,$sqlPrecioCompra);

$dataPrecioCompra = array(array('label'),array('precio'));

while($resultadoPrecioCompra = mysqli_fetch_array($queryPrecioCompra)) {
$dataPrecioCompra['label'][] = "'".$resultadoPrecioCompra['fecha']."'";
$dataPrecioCompra['precio'][] = ($resultadoPrecioCompra['precioCompra']) ? $resultadoPrecioCompra['precioCompra'] : 0;

}

$labelPrecioCompra = (sizeof($dataPrecioCompra['label']) > 1) ? implode(',',$dataPrecioCompra['label']) : $dataPrecioCompra['label'][0];

$preciosCompra = (sizeof($dataPrecioCompra['precio']) > 1) ? implode(',',$dataPrecioCompra['precio']) : $dataPrecioCompra['precio'][0];

// PRECIO DE VENTA

$sqlPrecioVenta = "SELECT precioVenta,fecha FROM registroegresos WHERE fecha BETWEEN '$desde' AND '$hasta' ORDER BY fecha ASC";

$queryPrecioVenta = mysqli_query($conexion,$sqlPrecioVenta);

$dataPrecioVenta = array(array('label'),array('precio'));

while($resultadoPrecioVenta = mysqli_fetch_array($queryPrecioVenta)) {
$dataPrecioVenta['label'][] = "'".$resultadoPrecioVenta['fecha']."'";
$dataPrecioVenta['precio'][] = ($resultadoPrecioVenta['precioVenta']) ? $resultadoPrecioVenta['precioVenta'] : 0;

}

$labelPrecioVenta = (sizeof($dataPrecioVenta['label']) > 1) ? implode(',',$dataPrecioVenta['label']) : $dataPrecioVenta['label'][0];

$preciosVenta = (sizeof($dataPrecioVenta['precio']) > 1) ? implode(',',$dataPrecioVenta['precio']) : $dataPrecioVenta['precio'][0];
